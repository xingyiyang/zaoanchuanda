...
sign_task = [
             u"今天是早起活动的最后一天，很开心在这段时间和大家相处，今天大家可以好好休息，统计好积分后会通知大家的获奖情况的。最后请发一段意见和感受给我们吧",           #星期天的任务   必须是文字型任务
             u"请早起在食堂吃一次早点吧，可以在食堂吃，也可以买了带到食堂吃",                            #星期1的任务
             u"拍一张树叶的照片",    #星期2的任务
             u"这是最后一周的早起签到了，感谢大家的参与和支持。今天请说说你这么多天的早起感受吧，也说说对我们的看法",          #星期3的任务  必须是文字型任务
             u"请吃一顿美美的早点，并截图发给我们" ,   #星期4的任务
     		 u"请吃一顿美美的早点，并截图发给我们",                     #星期5的任务
             u"请吃一顿美美的早点，并截图发给我们" ,           #星期6的任务	
             ]
'''
voice_id = [
            'Hh19xmgJAbRXPrY-LL9UCsz2OqTLU4AoCeNKlG4FMEf92JiLtWP-u3L96GE2eCvG',
            'L4r3qSiGQef6L6H-4iVjBGckaEnT-yLgTDZhUV4kOdwI7VllEGFa9SzieJhV8zbe',
            'gDhCWGhLlu-QHZkpB8jOpg5vSKoLMHvD7_gsRsY6KbGxDRV2B1U12bHxb0i3CLjN',
            'ATCf09xOgPJhN_n5QG-B7weOexspiXAexVmcPgCUC5AJZSSl9d7zVuJmPC4MztjL',
            'oyzEgi0NN661_aK719Mw8p5a1EY55pDYxbJNSQ8E2iuhOgzfxGFrYPjYq4mN3uTm',
            'jQ57pzdvm4KT49v_RCjbj4pVE0D1XH8FQNjNRJIAAZFIszV7SzFnno_l4O95XAt5',
            'GF5JXPE8FqRL4GnKQlzq1ybSi5FumFprb3Fol4tfs5ohVHAX04IOYvu1QIdPyLpO'
            ] 
'''
class WeixinInterface:
    ...
    def POST(self):
        ...
        if msgType == "text":
           ...
           # if week!=0 and week!=6:
            ...
            if content.find("+"):
                ...
                if temp2==u"早起活动" and len(tempstr)==4:
                    ...
                    if len(item)==0:
                        ...
                        return self.render.reply_text(fromUser,toUser,int(time.time()),u"你好！你已报名成功,请发送'早安'来进行进行今天的签到")
                    else:
                        return self.render.reply_text(fromUser,toUser,int(time.time()),u'你已经报过名了，请不要重复报名。')
                        
                elif len(tempstr)>=2 :
                    return self.render.reply_text(fromUser,toUser,int(time.time()),u"你发送的格式不对,请重新按照格式[早起活动+姓名+学号+电话]发送内容 如早起活动+小明+123+456")
            ...
            try:
                 ...
                 pass
            if len(item)==0:
                 return self.render.reply_text(fromUser,toUser,int(time.time()),u'你好！您目前还没有报名“早起活动”哦~可以发送[早起活动+姓名+学号+电话]报名参与。')
            ...
            if content.startswith(u'签到次数'):
                 if item.istoday:
                       return self.render.reply_text(fromUser,toUser,int(time.time()),u'你本月签到次数为:%d，今日已签到。'% (item.total))
                 else:
	                   return self.render.reply_text(fromUser,toUser,int(time.time()),u'你本月签到次数为:%d，今天还没有签到哦！'% (item.total))
          
            if content == u'签到积分':
                return self.render.reply_text(fromUser,toUser,int(time.time()),u'你好，你本月的早起积分为:%d，请继续努力！'%(item.all_record))
        ...
        #        return self.render.reply_text(fromUser,toUser,int(time.time()),u'安！假期与周末都不用签到哦，可以好好休息[愉快]')
            if hour<6 or hour >=8:
                return self.render.reply_text(fromUser,toUser,int(time.time()),u'不在签到时段，请在每天早上06:00~08:00签到哦，现在服务器时间%s'%fktime)
            ...
            if week==0 and len(content)>=10:
                 ...
                 return self.render.reply_text(fromUser,toUser,int(time.time()),u"恭喜小主今日完成签到任务[鼓掌]\n你是今天第%s个签到的人,请牢记你今日的内容!\
                                               \n%s\n%s\n%s\n%s\n\n姓名      签到时间  \n%s\n明天请继续输入'早安'开始新一天的挑战"%(sign_rank,str1,str2,str3,str4,tempstr))
